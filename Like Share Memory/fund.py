...
def process_all_stock_data(stock_data_list, stock_pair_list):
    stock_dict_list = []
    desc_dict = {
        "name" : u"key".encode('gb18030'),
        "index" : u"shmid".encode('gb18030'),
        "change" : u"nattch".encode('gb18030'),
        "rate" : u"status".encode('gb18030')
    }
    ...
    for stock_data in stock_data_list:
        ...
        stock_dict = {}
        stock_dict["name"] = stock_pair_list[i][1]
...
def print_all_stock_data(stock_dict_list):
   ...
   # print("_" * (name_max_len + index_max_len + change_max_len + rate_max_len + 10))
    ...
    for stock_dict in stock_dict_list:
        ...
        b"     " + stock_dict["index"].ljust(index_max_len) + b"     "\
        
        
        if stock_dict["change"].decode('gb18030') != u"nattch":
            if float(stock_dict["change"]) > 0:
                line += "+" + stock_dict["change"].ljust(change_max_len) + RESET
            elif float(stock_dict["change"]) < 0:
                line += stock_dict["change"].ljust(change_max_len) + RESET
            else:
                line += stock_dict["change"].ljust(change_max_len)
        else:
            line += stock_dict["change"].ljust(change_max_len)
        line += b"   "
        if stock_dict["rate"].decode('gb18030') != u"status":
            if float(stock_dict["rate"]) > 0:
                line += "+" + stock_dict["rate"].ljust(rate_max_len) + RESET
...
def main():
    ...
    stock_pair_list = cf.items("stock")
    ...
    while 1:
        ...
        if now> last_tick_time + int(options.delay):
            ...
            stock_dict_list = process_all_stock_data(stock_data_list, stock_pair_list)
